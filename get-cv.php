...
class PDF extends FPDF {
    ...
    function ChapterBody($data){
      ...
      $this->Cell(0,6,utf8_decode('Nacionalidade: '.$data->nationality));
    	$this->Ln();
      // Direction
      ...
      if($data->experiences) {
        ...
        for ($i=0; $i < count($data->experiences); $i++) { 
          $this->Cell(0,6,utf8_decode("- Cargo: ".(($data->experiences[$i]->position) ? $data->experiences[$i]->position." " : " ").(($data->experiences[$i]->company) ? " - Empresa: ".$data->experiences[$i]->company : "")));
    	    $this->Ln();
